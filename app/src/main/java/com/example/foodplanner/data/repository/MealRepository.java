...
public class MealRepository {
    ...
    private MealRepository(Context context) {
        ...
        remoteDataSource.init(context); // Initialize with context for network checking
    ...
    public Flowable<List<PlannedMeal>> getPlannedMealsByDateRange(long startDate, long endDate) {
        return plannedMealLocalDataSource.getPlannedMealsByDateRange(startDate, endDate);
    }

    public Completable addToPlanWithDate(Meal meal, String day, long plannedDate) {
        PlannedMeal plannedMeal = new PlannedMeal(meal, day, plannedDate);
        return plannedMealLocalDataSource.insertPlannedMeal(plannedMeal);
    }
}
